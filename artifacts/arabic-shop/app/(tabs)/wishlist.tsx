  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView
        ...
        contentContainerStyle={baseStyles.tabsContent}
      >
        {[{ id: "all", name: "الكل" }, ...collections].map((col) => {
          ...
          return (
            ...
            >
              <Text style={[baseStyles.tabText, { color: active ? "#fff" : colors.text }]}>{col.name}</Text>
            </TouchableOpacity>
          );
        })}
      </ScrollView>
      ...
      {collectionPickerItem && (
        <Modal transparent visible animationType="slide" statusBarTranslucent>
          <TouchableOpacity style={baseStyles.pickerOverlay} activeOpacity={1} onPress={() => setCollectionPickerItem(null)}>
